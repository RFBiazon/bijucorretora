...
export function normalizarProposta(json: PropostaJson) {
  // Se vier o objeto completo da proposta, use o campo resultado se existir
  const base = json.resultado && typeof json.resultado === 'object' && Object.keys(json.resultado).length > 0
    ? json.resultado
    : json

  return {
    id: json.id ?? base.id ?? "",
    status: json.status ?? base.status ?? "",
    valores: {
    ...
    criado_em: json.criado_em ?? json.created_at ?? base.criado_em ?? base.created_at ?? null,
  };
}
